...
public:
    ...
    // voice handling
    virtual int
    _stdcall OutputVoice_ProcessEvent(TOutVoiceHandle Handle, int EventID, int EventValue, int Flags) = 0;  // (GM)
...
public:
    ...
    virtual void
    _stdcall AddWave_32FM_32FS_Ramp(void *SourceBuffer, void *DestBuffer, int Length, float LVol, float RVol,
                                    float &LastLVol, float &LastRVol) = 0;

    // same, but takes a stereo source
    // note that left & right channels are not mixed (not a true panning), but might be later
    virtual void
    _stdcall AddWave_32FS_32FS_Ramp(void *SourceBuffer, void *DestBuffer, int Length, float LVol, float RVol,
                                    float &LastLVol, float &LastRVol) = 0;
    ...
    // see FHLS_ShowDialog
    virtual bool
    _stdcall LoadSample(TSampleHandle &Handle, char *FileName, PWaveFormatExtensible NeededFormat, int Flags) = 0;
    ...
    virtual void
    _stdcall DistWave_32FM(int DistType, int DistThres, void *SourceBuffer, int Length, float DryVol, float WetVol,
                           float Mul) = 0;
    ...
    virtual void _stdcall GetInBuffer(TPluginTag Sender, intptr_t Index,
                                      PIOBuffer IBuffer) = 0;    // returns (read-only) input buffer Index (or Nil if not available).
    virtual void _stdcall GetOutBuffer(TPluginTag Sender, intptr_t Index,
                                       PIOBuffer OBuffer) = 0;    // returns (add-only) output buffer Index (or Nil if not available). Use LockMix_Shared when adding to this buffer.


    // output voices (VFX "voice effects")
    virtual TOutVoiceHandle
    _stdcall TriggerOutputVoice(TVoiceParams *VoiceParams, intptr_t SetIndex, intptr_t SetTag) = 0;  // (GM)
    ...
    virtual int _stdcall OutputVoice_ProcessEvent(TOutVoiceHandle Handle, intptr_t EventID, intptr_t EventValue,
                                                  intptr_t Flags) = 0;  // (GM)
...
const int FPF_GetChanCustomShape =
        1 << 3;   // generator will use the extra shape sample loaded in its parent channel (see FPD_ChanSampleChanged)
const int FPF_GetNoteInput =
        1 << 4;   // plugin accepts note events (not used yet, but effects might also get note input later)
const int FPF_WantNewTick = 1
        << 5;   // plugin will be notified before each mixed tick (& be able to control params (like a built-in MIDI controller) (see NewTick))
const int FPF_NoProcess =
        1 << 6;   // plugin won't process buffers at all (FPF_WantNewTick, or special visual plugins (Fruity NoteBook))
...
const int FPF_TimeWarp = 1
        << 13;  // supports timewarps, that is, can be told to change the playing position in a voice (direct from disk music tracks, ...) (not used yet)
const int FPF_MIDIOut =
        1 << 14;  // plugin will send MIDI out messages (only those will be enabled when rendering to a MIDI file)
const int FPF_DemoVersion = 1 << 15;  // plugin is a demo version, & the host won't save its automation
const int FPF_CanSend =
        1 << 16;  // plugin has access to the send tracks, so it can't be dropped into a send track or into the master
const int FPF_MsgOut =
        1 << 17;  // plugin will send delayed messages to itself (will require the internal sync clock to be enabled)
const int FPF_HybridCanRelease = 1
        << 18;  // plugin is a hybrid generator & can release its envelope by itself. If the host's volume envelope is disabled, then the sound will keep going when the voice is stopped, until the plugin has finished its own release
const int FPF_GetChanSample =
        1 << 19;  // generator will use the sample loaded in its parent channel (see FPD_ChanSampleChanged)
const int FPF_WantFitTime =
        1 << 20;  // fit to time selector will appear in channel settings window (see FPD_SetFitTime)
...
const int FPF_Type_FullGen =
        FPF_Generator | FPF_GetNoteInput | FPF_NewVoiceParams;   // for a full standalone generator (Gen_Render)
const int FPF_Type_HybridGen =
        FPF_Type_FullGen | FPF_UseSampler | FPF_NewVoiceParams;  // for an hybrid generator (Voice_Render)
const int FPF_Type_Visual =
        FPF_NoProcess | FPF_NewVoiceParams;                      // for a visual plugin that doesn't use the wave data
